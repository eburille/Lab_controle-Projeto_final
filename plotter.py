...
def plotar_historico(nome_arquivo=None):
    ...
    plt.plot(tempo, posicao, label="Posição medida (rad)", color="b", linewidth=2)
    plt.plot(tempo, controle, label="Sinal de controle (0-100 %)", color="r", linewidth=2)

    
    # Estilização do Gráfico (Dark/Modern Mode condizente com a GUI)
    plt.title(f"Teste do controlador", fontsize=12, fontweight='bold', color='#121212')
